...
type EditorWalletMenuProps = {
    handleCloseMenu: () => void
}

type MenuAddressProps = {
    address: string
}
...
type MenuCopyAddressProps = {
    address: string
}

type MenuViewExplorerProps = {
    address: string
}
...
export default function EditorWalletMenu({ handleCloseMenu }: EditorWalletMenuProps) {
    return( 
...
function MenuAddress({ address}: MenuAddressProps) {
    return <h1 className="text-2xl">{condenseAddress(address)}</h1>
}

function MenuOptions({ address }: MenuOptionsProps) {
    return (
...
function MenuCopyAddress({ address }: MenuCopyAddressProps) {
...
function MenuViewExplorer({ address }: MenuViewExplorerProps) {
    const handleViewExplorer = () => {
        const viewExploreAddress = "https://etherscan.io/address/" + address
        window.open(viewExploreAddress, "_blank") || (window.location.href = viewExploreAddress)
    }
...
function MenuViewExploreImage() {
    return <img src="/images/logos/etherscan.svg" className="w-full" /> 
}
